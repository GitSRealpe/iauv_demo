...
from clustering_stuff.srv import Cluster, ClusterResponse
from clustering_stuff.msg import ClusterObjInfo
...
def find_optimal_clusters(data):
    ...
    )  # Adding 2 because the range starts from 2
...
def publish_object_info(object_info_list):
    object_info_pub = rospy.Publisher(
        "/clustered_point_cloud", ClusterObjInfo, queue_size=10
    )
    object_cluster_pub = rospy.Publisher(
        "/clustered_point_cloud_visual", PointCloud2, queue_size=10
    )
    rate = rospy.Rate(10)  # 10 Hz

    print("going to publish object_info")
    print("size of object_info_list", len(object_info_list))
...
def clustering_server():
    ...
    rospy.Subscriber("/octomap_point_cloud_centers", PointCloud2, pointcloud_callback)
